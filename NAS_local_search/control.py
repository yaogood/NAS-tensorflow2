...
class StateSpace:
    ...
    def get_local_state_space(self, count, states):
        result = []
        for id, value in enumerate(states):
            state = self[id]
            value_idx = state['value_map_'][value]
            if (count >> id) & 0x00000001 == 1:
                add = int(count / (2 ** len(states)) + 1)
                new_value = state['index_map_'][(value_idx - add) % state['size']]
                result.append(self.embedding_encode(id, new_value))
            else:
                result.append(self.embedding_encode(id, value))
        return result

    def get_local_state_space_add(self, count, states):
        result = []
        for id, value in enumerate(states):
            state = self[id]
            value_idx = state['value_map_'][value]
...
class Controller:
    ...
    def train_step(self):
        ...
        with self.policy_session.as_default():
            ...
            if global_step > 50 and global_step % 10 == 0:
